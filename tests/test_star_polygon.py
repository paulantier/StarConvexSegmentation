...
def test_star_polygon_assemble_patches(star_polygon_model, sample_image):
    ...
    for patch in patches:
        with torch.no_grad():
            processed_patch = star_polygon_model.polygon_model(patch.to(star_polygon_model.device))
            processed_patches.append(processed_patch.to("cpu"))
    
    assembled_image = star_polygon_model._assemble_patches(processed_patches, sample_image.shape)
